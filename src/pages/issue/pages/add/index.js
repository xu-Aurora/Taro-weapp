...
export default class Index extends PureComponent {
  ...
  constructor() {
    ...
    this.state = {
      ...
      MinAmt: "", //最低价格
      MaxAmt: "", //最高价格
      Tel: userInfoData && JSON.parse(userInfoData).Mobile, //手机号码
      Contact: userInfoData && JSON.parse(userInfoData).RealName, //联系人
      Gender: "男", //性别
      selector: ["男", "女"],
    ...
    this.setState(
      ...
      () => {
        ...
        this.state.AreaZone.forEach(ele => {
          if (
            e.detail.value[0] ==
            (ele.AreaName.length == 2 ? `${ele.AreaName}市` : ele.AreaName)
          ) {
        ...
        api.areaZone({ ProvinceName: e.detail.value[0] }).then(res => {
          if (res.data.code === 200) {
            let City;
            if (
              e.detail.value[0] === "北京市" ||
              e.detail.value[0] === "天津市" ||
              e.detail.value[0] === "上海市" ||
  ...
  render() {
    ...
    return (
      <View className="issueAdd">
        <CoverView
          className="nav"
          style={{ height: `calc(${status}px + ${navHeight}px)` }}
        >
          <CoverView
            className="status"
            style={{ height: `${status}px` }}
          ></CoverView>
          <CoverView className="navbar" style={{ height: `${navHeight}px` }}>
            <CoverView className="btn_box">
              <CoverView
                className="back-icon"
                onClick={this.goPage.bind(this, "back")}
              >
        ...
        >
          ...
          <View className="footer">
            <AtButton disabled={clickBtn} onClick={this.release} type="primary">
              发布
            </AtButton>
          </View>
        </View>
