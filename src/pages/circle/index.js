...
import { AtButton, AtActionSheet, AtActionSheetItem, AtModal, AtModalHeader, AtModalContent, AtModalAction,AtTabs, AtTabsPane } from 'taro-ui'
...
export default class Index extends PureComponent {
  ...
  componentWillMount() {
    ...
    api.ifZSaccount({
      ...
      if (res.data.code === 305) {                  // 未开户，仍可以使用其他卡
        this.setState({
          isZS: true
        })
      } 
          ...
          <View style={{marginTop: titleHeight, minHeight: `calc(100vh - ${titleHeight})`}}>
            ...
            <View className='top'>
              ...
              <View className='balance'>
                <View className='left'>
                  <View>{ splitThousand(accountDatas.Amt ? accountDatas.Amt : 0) }</View>
                  <View>账户余额<Text style={{fontSize: '20rpx'}}> (元)</Text></View>
                </View>
                {
              ...
              <View className='earnings'>
                ...
                <View className='left common' onClick={this.goIncome.bind(this, accountDatas.CircleId)}>
                  <Image src={`${imgUrl}income.png`} />
                  <View>
                    <View>+{ accountDatas.SumProfit == 0 ? '0.00' : accountDatas.SumProfit }</View>
                ...
                <View className='right common' onClick={this.goRepay}>
                  ...
                  <View>
                    ...
                    <View>贷款金额<Text style={{fontSize: '20rpx'}}> (元)</Text></View>
                  </View>
                </View>
              </View>
            </View>
            ...
            <AtTabs current={current} tabList={tabList} onClick={this.handleClick.bind(this)}>
              <AtTabsPane current={current} index={0}>
                {
                  (JSON.stringify(pzParkArr) !== '[]') && pzParkArr !== null && pzParkArr.map(ele => {
                    return (
                      <View className='car_list' 
                        key={ele.ParkingId}
                        onClick={this.goDetail.bind(this,ele.ParkingId,ele.State)}
                      >
                        <View className='item'>
                          <View>
                          ...
                          <View>
                            ...
                            </View>
                            {
                              ...
                              <View onClick={this.backBuy.bind(this, ele)}>
                                <AtButton type='primary' loading={disabled} size='small'>申请回购</AtButton>
                              </View>
                            }
                            {
                              ele.State == 6 &&
                              <View onClick={this.cancel.bind(this, ele.ParkingId)}>
                ...
                <View className='footer1'
                  style={{display: (JSON.stringify(pzParkArr) !== '[]' && pzParkArr !== null)? 'flex' : 'none'  }}
                >
                  <View>
              ...
              <AtTabsPane current={current} index={1}>
                {
                  (JSON.stringify(qzParkArr) !== '[]') && qzParkArr !== null && qzParkArr.map(ele => {
                    return (
                      <View 
                        onClick={this.goDetail.bind(this,ele.ParkingId,ele.State)} 
                        className='car_list' key={ele.OwnnerId}
                      >
                        <View className='item'>
                          <View>
                            <View className='itemTitle_wrap'>
                ...
                <View className='footer1'
                  style={{display: (JSON.stringify(qzParkArr) !== '[]' && pzParkArr !== null) ? 'flex' : 'none'  }}
                >
                  <View>
                    <View className='line'></View>
                    <View className='text'>已经到底啦</View>
            ...
            >
              {
                isZS && 
                <AtActionSheetItem onClick={this.goPage.bind(this, 'bank_bind_update')}>
                  修改绑定银行卡
                </AtActionSheetItem>
              }


              <AtActionSheetItem onClick={this.goPage.bind(this, 'invite_join')}>
                邀请会员加入
              </AtActionSheetItem>
              <AtActionSheetItem onClick={this.goPage.bind(this, 'al_bus_detail')}>
                查看商圈详情
              </AtActionSheetItem>
            </AtActionSheet>
            ...
            <View className='c_modal'>
              ...
              >
                  <View className='modal_header'>
                    <AtModalHeader>您可以通过手机银行、网上银行、银行柜台等渠道进行充值。</AtModalHeader>
                  </View>
                  <AtModalContent>
                    <View className='modal1'>
                      <View className='mar_bot'>请通过该商圈所绑定的银行卡：</View>
                      <View className='mar_bot'>
                        { JSON.stringify(accountDatas) !== '{}'&&accountDatas.accNo && `${accountDatas.accNo.slice(0,4)} **** **** ${accountDatas.accNo.slice(-4)}` }
                      </View>
                      <View className='mar_bot'>向该商圈账号进行转账汇款</View>
                      <View className='col'>户名：{ Taro.getStorageSync('userInfo')&&JSON.parse(Taro.getStorageSync('userInfo')).userName }</View>
                      <View className='col'>签约账号：{ accountDatas.CircleAccountNo }</View>
                      <View className='col'>开户行：{ `浙商银行` }</View>
                    </View>

                  </AtModalContent>
                  <AtModalAction>
                    <Button onClick={() => this.setState({isOpened2: false})}>确定</Button>
                  </AtModalAction>
                </AtModal>
            </View>
          </View>
