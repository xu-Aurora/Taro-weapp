...
export default class Index extends PureComponent {
  ...
  goPage = () => {
    // 提交订单之前判断是否有上传证件
    const userInfoData = Taro.getStorageSync("userInfoData")
      ? JSON.parse(Taro.getStorageSync("userInfoData"))
      : "";
  ...
  parkCode(data) {
    ...
    if (data !== undefined && JSON.stringify(data) !== "{}") {
      if (data.BuyBackModel.Usufruct == 0) {
        code = `${data.ParkingCode.slice(0, 1)} ***** ${data.ParkingCode.slice(
          -1
        )}`;
      } else {
  ...
  agree() {
    ...
    if (!btn) {
      ...
      type === "loan_buy" ? "个人信息查询授权书" : "销售协议";
      toast(`请阅读${text}`, "none", 2000);
    }
  }
  ...
  async componentDidShow() {
    const { datas, Price, type } = this.$router.preload;
    this.setState(
      {
        datas: datas,
        price: Price,
        isSwitch: datas.Bond > 0 ? true : false,
        type: type,
        disabled: get("isAgree1") ? false : true,
        isAgree: get("isAgree1") ? "同意" : "未同意"
      },
      () => {
        if (type === "loan_buy") {
          this.signAgreeMent();
    ...
    return (
      <View className="full_buy">
          ...
          >
            <View className="carInfo">
              ...
              <View className="carInfo">
                ...
                <View>
                  ...
                  {/* <Text>
                    <Text decode className='col1'>类别 :&nbsp;</Text>
                    <Text className='col2'>{ this.type(datas.ParkingTraitType) }</Text>
                  </Text> */}
                </View>
                ...
                <View>
                  <View>
                    ...
                    <Text className="col2">
                      {datas.EffectiveTime && datas.EffectiveTime.split(" ")[0]}
                    </Text>
                  </View>
                </View>
            ...
            <View className="earnest" style={{ display: display }}>
              <AtList>
                ...
                />
                <View>
                  <AtListItem title="定金金额 (元)" extraText={String(price)} />
                </View>
              </AtList>
            </View>
            {/* 定金说明 */}
            <View className="explain" style={{ display: display }}>
              ...
              <Text>
                支付定金后，资产将为您锁定。您须自订单日期起7日内支付尾款，否则订单将过期失效，资产自动解锁，过期失效订单的已付定金不予退还。若客户违约，定金不予退还。
              </Text>
            </View>
          </View>
        </View>
        ...
        <View className="footer2">
          {type === "loan_buy" ? (
            <AtButton type="primary" disabled={disabled} onClick={this.goPage}>
              提交贷款申请
            </AtButton>
          ) : (
        ...
        >
          <AtModalHeader>请阅读并同意</AtModalHeader>
          <AtModalContent>
          ...
          {/* <AtModalAction>
            <Button onClick={this.noAgree.bind(this)}>不同意</Button>
            <Button
