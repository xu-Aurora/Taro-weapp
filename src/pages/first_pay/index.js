...
export default class Index extends PureComponent {
              ...
              this.setState(
                {
                  ...
                  firstAmount: Subtr(Price, data[0].loanAmount),
                  scale: parseInt(
                    numDiv(Subtr(Price, data[0].loanAmount), Price) * 100
                  ),
                  scale1: parseInt(
                    numDiv(Subtr(Price, data[0].loanAmount), Price) * 100
                  ) // 首付比例不等低于的值
  ...
  componentWillMount() {
    ...
    this.setState({
      datas,
      isAgree1: get("isAgree1") ? "同意" : "未同意",
      isAgree2: get("isAgree2") ? "同意" : "未同意",
      disabled: get("isAgree1") && get("isAgree1") && true
    });

    const asyncHttp = async () => {
      await this.getAccountAmt();
      await this.getLoanScheme(
        datas.OrderCode,
        accMul(datas.Price, 0.2),
        datas.Price
      );
    };
    asyncHttp();
  ...
  render() {
    ...
    const rate = accAdd(
      accMul(loanData.callDatumRate, 100),
      loanData.spread / 100
    );
    const interest = numDiv(
      accMul(loanData.loanAmount * numDiv(rate, 12), loanData.loanTerm),
      100
    ).toFixed(2);
    ...
    return (
      <View className="first_pay">
        <View>
          ...
          >
            ...
            <View className="item">
              <Text>
                首付金额{" "}
                <Text style={{ fontSize: "24rpx", color: "#3E3E3E" }}>
                  (元)
                </Text>
              </Text>
            ...
            <View className="inp_text">
              ...
              <View className="tips" style={{ display: tipsStyle }}>
                提示：首付比例不能少于{scale1}%
              </View>
            </View>
            ...
            <View
              className="item"
              style={{ padding: "32rpx 0rpx 32rpx 32rpx" }}
            >
              <Text style={{ color: "#3E3E3E", fontSize: "32rpx" }}>
                贷款金额 <Text style={{ fontSize: "24rpx" }}>(元)</Text>
              </Text>
            ...
            {loanData.rateType == 2 && (
              <View>
                ...
                <View className="item item1">
                  <Text>
                    预估利率 <Text style={{ fontSize: "24rpx" }}>(%)</Text>
                  </Text>
                  <Text>{rate}</Text>
                  <Image
                    src={`${imgUrl}icon_info_line.png`}
                    onClick={this.showVis}
                  />
                </View>
                <View className="vis" style={{ display: display2 }}>
                  ...
                  <View>
                    <Text>
                      挂靠利率 <Text style={{ fontSize: "24rpx" }}>(%)</Text>
                    </Text>
                    <Text>{accMul(loanData.callDatumRate, 100)}</Text>
                  </View>
                  <View>
                    <Text>
                      点差 <Text style={{ fontSize: "24rpx" }}>(100bp=1%)</Text>
                    </Text>
        ...
        <View className="f_box">
          <View className="footer_1">
            <View>
              <View>
                <Text>
                  支付金额 <Text style={{ fontSize: "24rpx" }}>(元)</Text>
                </Text>
                {datas.IfPayBond != -1 ? (
                  <Text decode>
                    ¥&nbsp;{splitThousand(Subtr(firstAmount - datas.IfPayBond))}
                  </Text>
                ) : (
                  <Text>
                    {firstAmount == "" ? 0 : splitThousand(firstAmount)}
                  </Text>
                )}
              ...
              {datas.IfPayBond != -1 && (
                <View>（实付金额为首付金额扣除已付定金）</View>
              )}
            </View>
            <View onClick={this.goPay.bind(this, datas.OrderCode, "5")}>
              <AtButton type="primary" size="small" disabled={disabled}>
                支付
              </AtButton>
            </View>
          </View>
        ...
        <AtModal isOpened={modal1}>
          ...
          <AtModalAction>
            <Button onClick={() => this.setState({ modal1: false })}>
              不同意
            </Button>
            <Button
              disabled={btn1}
