  ...
  return (
    <View className='boxs'>
      <View className=' protocol'>
        ...
        <View className='indent mt'>1.1 资产通的具体服务内容由资产通根据实际情况提供，内容主要包括车位使用权益在线交易服务、用户与银行支付系统提供媒介服务以及其他用户服务。 （1）车位使用权属在线交易服务提供的内容（包括但不限于用户的单位名称、联系人、联系信息、业务产品的描述和说明、相关图片、相关业务收费等）的信息均由用户在线提交，相关用户依法应对其发布的任何信息承担全部责任，资产通对此类信息的准确性、完整性、合法性或真实性均不承担任何责任。 （2）银行支付系统利用科技手段嵌入资产通系统，为资产通用户提供融资服务、资金存管与监管、车位信息的真实性及有效性进行验证、对车位流转状态进行判别等服务，具体权利责任及风险认定视用户使用的银行支付系统相关协议声明而定。资产通在用户与银行支付系统间为媒介角色存在，资产通在其中不承担任何法律责任。</View>
        ...
        <View className='indent mt'>2.3 用户保证并承诺通过资产通平台进行交易的车位和资金来源合法。</View>
